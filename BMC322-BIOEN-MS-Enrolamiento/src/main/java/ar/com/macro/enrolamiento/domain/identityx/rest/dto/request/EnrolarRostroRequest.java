...
@AllArgsConstructor
@NoArgsConstructor
public class EnrolarRostroRequest {

    @Schema(description = "Id Usuario de Identity-X", required = true)
	@NotBlank(message = "{api.validacion.no.blank.message}")
	@Pattern(regexp = "^[A-Za-z0-9]+$", message = "{api.validacion.EnrolarRostroRequest.idusuario.alfa.pattern.message}")
	@Size(min = 8, max = 15, message = "{api.validacion.EnrolarRostroRequest.idusuario.size.pattern.message}")
