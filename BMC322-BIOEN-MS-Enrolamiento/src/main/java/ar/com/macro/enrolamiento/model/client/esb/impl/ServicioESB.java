...
@Primary
@Component("enrolamientoServicioESB")
@Slf4j
...
public class ServicioESB {
    ...
    public ResponseEntity<String> get(String servicio, MultiValueMap<String, String> parameters,
        ...
        return this.invocarServicio(uriComponents.toString(), HttpMethod.GET,
                httpHeaders != null ? new HttpEntity<>(httpHeaders) : new HttpEntity<>(new HttpHeaders()));
    }
    ...
    public ResponseEntity<String> post(String servicio, String body, Map<String, String> headers) {
        ...
        HttpHeaders httpHeaders = this.generarCabeceras(headers);
        return this.invocarServicio(uriComponentsBuilder.build().toString(), HttpMethod.POST,
                httpHeaders == null ? new HttpEntity<>(body) : new HttpEntity<>(body, httpHeaders));
    }

    public ResponseEntity<String> put(String servicio, String body, Map<String, String> headers) {
        UriComponentsBuilder uriComponentsBuilder = UriComponentsBuilder.fromUriString(servicio);
        HttpHeaders httpHeaders = this.generarCabeceras(headers);
        return this.invocarServicio(uriComponentsBuilder.build().toString(), HttpMethod.PUT,
                httpHeaders == null ? new HttpEntity<>(body) : new HttpEntity<>(body, httpHeaders));
    }
    ...
    private String getCuerpoComoCadena(HttpEntity<?> entidadHttp) {
        ...
        if (entidadHttp.getBody() != null) {
            cuerpoRequest = (String) Stream.of(entidadHttp.getBody().toString().split("\"")).map((elemento) -> {
    ...
    private ResponseEntity<String> invocarServicio(String uri, HttpMethod method, HttpEntity<?> entidadHttp) {
        ...
        ResponseEntity<String> var12;
        try {
            start = System.currentTimeMillis();
            MDC.put("ESBEndpointDuration", "");
            log.info("Request ESB url: {}, Request ESB body: {}, Request ESB headers: {}",
                    new Object[] { uri, this.getCuerpoComoCadena(entidadHttp), entidadHttp.getHeaders() });
            response = this.restTemplate.exchange(uri, method, entidadHttp, String.class, new Object[0]);
            duration = System.currentTimeMillis() - start;
            log.info("Response ESB url: {}, Response ESB duracion: {} ms, {}",
                    new Object[] { uri, duration, duration });
            if (response == null || response.getStatusCode() == null) {
                throw new ComunicacionFallidaException(ErrorCode.CODIGO_GENERAL_ESB_NO_DISPONIBLE,
                        ErrorMessage.ESB_NO_DISPONIBLE.mensaje());
            }

            RespuestaHttp.builder().httpCodigo(response.getStatusCodeValue())
                    .httpDescripcion(response.getStatusCode().getReasonPhrase()).build();
            log.info("Response ESB headers: {}", response.getHeaders());

            return response;
        } catch (HttpServerErrorException var23) {
            duration = System.currentTimeMillis() - start;
            log.info("Response ESB HttpServerErrorException url: {}, duracion: {} ms, {}",
                    new Object[] { uri, duration, duration });
            RespuestaHttp.builder().httpCodigo(var23.getRawStatusCode())
                    .httpDescripcion(var23.getStatusText()).build();
            var12 = new ResponseEntity<String>(var23.getResponseBodyAsString(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (ResourceAccessException var24) {
            duration = System.currentTimeMillis() - start;
            log.info("Response ESB ResourceAccessException url: {}, duracion: {} ms, {}",
                    new Object[] { uri, duration, duration });
            throw new ComunicacionFallidaException(ErrorCode.CODIGO_GENERAL_ESB_NO_DISPONIBLE, var24.getMessage(),
                    var24);
        } finally {
    ...
    public Map<String, String> configurarCabeceraServicio() {
        ...
        headers.put(HeaderConstants.HEADER_X_AUTHORIZATION_IMPERSONATIONLEVEL,
                HeaderConstants.HEADER_X_AUTHORIZATION_IMPERSONATION_LEVEL_VALUE_SERVICE);
        return headers;
    }
    ...
    public Map<String, String> headerApplication() {
        Map<String, String> headers = new HashMap<>();
        headers.put(HeaderConstants.HEADER_X_AUTHORIZATION_IMPERSONATIONLEVEL,
                HeaderConstants.HEADER_X_AUTHORIZATION_IMPERSONATION_LEVEL_VALUE_APPLICATION);
        headers.put(HeaderConstants.HEADER_X_APPLICATION_USER_PRINCIPAL_NAME,
                catalogoDeServicios.getUsuarioNivelImpersonalizacionApplication());
        return headers;
    }
    ...
    private ProcesadorRespuesta procesarRespuestaConsumoServicio(ResponseEntity<String> respuesta) {
        return respuesta != null
                ? new ProcesadorRespuesta(
                        (RespuestaStrategy) this.estrategiaMap.get(String.valueOf(respuesta.getStatusCodeValue())))
                : new ProcesadorRespuesta((RespuestaStrategy) this.estrategiaMap.get("NUll"));
    }
    ...
    public ResponseEntity<String> ejecutarGetAlESB(String servicio, MultiValueMap<String, String> parameters,
            Map<String, String> headers) {
        ResponseEntity<String> responseEntity = null;
